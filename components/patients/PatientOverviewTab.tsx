  ...
  return (
    <div>
      ...
      {editing ? (
        ...
        <button
          onClick={startEdit}
          className="mt-1 w-full text-start text-sm px-2 py-1.5 rounded-md border border-dashed border-border hover:border-primary hover:bg-muted/50 transition-colors min-h-[34px]"
        >
          {value ? (
            <span>{value}</span>
          ) : (
...
function ChronicConditionsSection({
  ...
  const conditionTypes = [
    { value: "diabetes", label: t("conditionFullDiabetes") },
    { value: "hypertension", label: t("conditionFullHypertension") },
    { value: "cardiac", label: t("conditionFullCardiac") },
    { value: "thyroid", label: t("conditionFullThyroid") },
    { value: "other", label: t("conditionFullOther") },
  ]
  ...
  async function handleAdd() {
    ...
    try {
      ...
      toast.error(t("errorAddCondition"))
    } finally {
      setAdding(false)
    }
  }
  ...
  async function handleRemove(conditionId: string) {
    ...
    try {
      ...
      toast.error(t("errorRemoveCondition"))
    } finally {
      setRemovingId(null)
    }
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <Label className="text-xs text-muted-foreground">{t("conditionsLabel")}</Label>
        {!showAddForm && (
          <button
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        {conditions.length === 0 && !showAddForm && (
          <p className="text-xs text-muted-foreground">{t("noConditionsRecorded")}</p>
        )}
        {conditions.map((c) => (
          <div
            key={c.id}
          ...
          >
            <span>{t(CONDITION_LABELS[c.type] ?? CONDITION_LABELS.other)}</span>
            <button
              onClick={() => handleRemove(c.id)}
              disabled={removingId === c.id}
              className="hover:opacity-70 transition-opacity disabled:opacity-40 -ms-0.5"
      ...
      {showAddForm && (
        <div className="border rounded-md p-3 bg-muted/30 space-y-2">
          <div>
            <Label className="text-xs mb-1 block">{t("conditionTypeLabel")}</Label>
            <select
              value={newType}
          ...
          <div>
            <Label className="text-xs mb-1 block">{t("conditionNotesLabel")}</Label>
            <Input
              value={newNotes}
          ...
          <div className="flex gap-2">
            <Button size="sm" onClick={handleAdd} disabled={adding} className="h-7 text-xs">
              {adding ? tCommon("saving") : tCommon("save")}
            </Button>
            <Button
              size="sm"
              variant="outline"
...
export function PatientOverviewTab({
  ...
  async function saveAllergies(value: string) {
    try {
      ...
      toast.error(t("errorSaveAllergies"))
    }
  }
    ...
    <div className="p-4 space-y-6 max-w-2xl">
      ...
      {showAlert && (
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3">
          ...
          <p className="text-sm text-amber-800">
            {t("alertLastVisit", { days: daysSinceLastVisit })}
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-muted/40 rounded-lg p-3 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">{t("statTotalVisits")}</p>
        </div>
        <div className="bg-muted/40 rounded-lg p-3 text-center">
          <p className="text-2xl font-bold">{patient.totalSpent.toLocaleString("ar-EG")}</p>
          <p className="text-xs text-muted-foreground mt-0.5">{t("statTotalPaid")}</p>
        </div>
        <div className="bg-muted/40 rounded-lg p-3 text-center">
          <p className="text-sm font-semibold leading-snug">{lastVisitFormatted}</p>
          <p className="text-xs text-muted-foreground mt-0.5">{t("statLastVisit")}</p>
        </div>
      </div>
      ...
      <InlineEditField
        label={t("permanentNotesLabel")}
        value={patient.permanentNotes}
        placeholder={t("permanentNotesPlaceholder")}
